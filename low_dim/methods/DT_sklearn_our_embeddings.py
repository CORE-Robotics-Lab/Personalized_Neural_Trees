...
checkpoint = torch.load(
                '/home/Anonymous/PycharmProjects/bayesian_prolo/saved_models/HRI/toy_interpretable_2.tar')

x = []
for l,each_ele in enumerate(x_data):
    x.append(each_ele[2:])
    num = l % 20
    x[-1].extend(checkpoint['embeddings'][num])
...
x_test = []
for l, each_ele in enumerate(x_data_test):
    x_test.append(each_ele[2:])
    num = l % 20
    x_test[-1].extend(checkpoint['test_embeddings'][num])
