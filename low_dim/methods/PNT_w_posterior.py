...
created on February 27,  2020 by Rohan Paleja
"""
...
for da_num in range(0,3):
    ...
    optimizer = torch.optim.SGD([{'params': list(nn.ddt.parameters())[:-1]}, {'params': nn.ddt.bayesian_embedding.parameters(), 'lr': .01}], lr=learning_rate)
    ...
    for epoch in range(epochs):  # loop over the dataset multiple times
        ...
        for i in range(num_schedules):
            ...
            for each_t in range(chosen_schedule_start, chosen_schedule_start + 20):
                ...
                loss = (torch.Tensor(distributions[schedule_num]) - output) ** 2 * 10
                loss1 = loss.sum()
                out = nn.forward_action(x[each_t], embedding=list(distributions[schedule_num]))
                loss2 = F.cross_entropy(out, y[each_t].long()) * 30
